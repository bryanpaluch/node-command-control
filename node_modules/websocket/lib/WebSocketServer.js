...
var WebSocketServer = function WebSocketServer(config) {
    ...
    //replace connections array with a connections keypair object
    this.connections = {};
    this.idlist = []; 
	if (config) {
        this.mount(config);
	...
	var seed = "lkaz" + Date();	
	for(var kl=0; kl < this.config.idBufferSize; kl++)
	{
	this.idlist.push(crypto.createHash('md5').update(seed + kl).digest("hex"));
	}
};
...
WebSocketServer.prototype.closeAllConnections = function() {
    Object.keys(this.connections).forEach(function(connection) {
        connection.close();
    });
};
...
WebSocketServer.prototype.handleRequestAccepted = function(connection) {
    ...
    //create unique key for the connection and put it into the object
    var connid = this.idlist.shift();	
...
WebSocketServer.prototype.handleConnectionClose = function(connection, closeReason, description) {
        delete this.connections[connection.connid];
	this.idlist.push(connection.connid);
